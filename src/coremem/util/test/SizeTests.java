...
public class SizeTests
{
	...
	public void testNIOBuffers()
	{
		...
		assertEquals(2 * 11, Size.of(ByteBuffer.allocateDirect(2 * 11)
																						.asCharBuffer()));
		assertEquals(2 * 11, Size.of(ByteBuffer.allocateDirect(2 * 11)
																						.asShortBuffer()));
		assertEquals(4 * 11, Size.of(ByteBuffer.allocateDirect(4 * 11)
																						.asIntBuffer()));
		assertEquals(8 * 11, Size.of(ByteBuffer.allocateDirect(8 * 11)
																						.asLongBuffer()));
		assertEquals(4 * 11, Size.of(ByteBuffer.allocateDirect(4 * 11)
																						.asFloatBuffer()));
		assertEquals(8 * 11, Size.of(ByteBuffer.allocateDirect(8 * 11)
